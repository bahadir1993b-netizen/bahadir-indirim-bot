...
KEY=os.environ['SUPABASE_SERVICE_KEY']; TOKEN=os.environ['TELEGRAM_BOT_TOKEN']
...
def sb(method,path,**kw):
    ...
    url=f'{BASE}/rest/v1/{path.lstrip("/")}';r=requests.request(method,url,headers=h,timeout=15,**kw)
...
def canonical(u):
    try:
        p=urlparse(u or '');return urlunparse(('https',p.netloc.lower(),p.path.rstrip('/'),'','',''))
    except:return u
...
def _prices(soup,selectors):
    out=[]
    for sel,attr in selectors:
        for e in soup.select(sel)[:10]:
            p=num(e.get(attr) if attr else e.get_text(' ',strip=True))
            if p:out.append(p)
    return out

def parse_html(body,expected=None,site=''):
    soup=BeautifulSoup(body,'html.parser');text=re.sub(r'\s+',' ',soup.get_text(' ',strip=True));low=text.lower()
    if any(x in low for x in ['stokta yok','stokta bulunmuyor','ürün tükendi','urun tukendi','currently unavailable','out of stock','sold out']):return {'oos':True}
    title='';image=''
...
def http_check(url,expected=None):
    try:
        ...
        return parse_html(r.text,expected,site_of(r.url or url))
    except:return None

def browser_check(page,url,expected=None):
    try:
        page.goto(url,wait_until='domcontentloaded',timeout=14000);page.wait_for_timeout(1200)
        return parse_html(page.content(),expected,site_of(page.url or url))
    except:return None
...
def history(url):
    ...
    except Exception as e:print(f'GEÇMİŞ HATA: {type(e).__name__}: {e}');return []
...
def reference(current,hist,page_old,stored_prev=None):
    ...
    return (float(min(local)),'history/page') if local else (None,'none')

def maybe_write_history(url,site,current,hist):
    last=hist[0] if hist else None;write=not last
    if last:
        lp,dt=last
    ...
    if write:sb('POST','price_history',json={'price':current,'product_url':canonical(url),'site':site,'recorded_at':datetime.now(timezone.utc).isoformat()});STATS['history_writes']+=1
...
def main():
    ...
    rows=load_catalog();STATS['catalog']=len(rows);print(f'KATALOG: {len(rows)} ürün');browser_used=0
    with sync_playwright() as pw:
        browser=pw.chromium.launch(headless=True,args=['--disable-blink-features=AutomationControlled']);page=browser.new_page()
        for row in rows:
            try:
                STATS['checked']+=1;url=canonical(row['product_url']);site=row['site'];expected=num(row.get('current_price'));info=http_check(url,expected)
