...
# Kaynaklar sırayla işlensin. Mevcut ThreadPoolExecutor kullanımı varsa
# kaynak/mesaj gönderim sırasını bozmaması için basit sıralı executor kullan.
...
# Aynı kaynak çalışmasında gereksiz mesaj yığılmasını önle.
if "for b in blocks[:SOURCE_LIMIT]:" not in s:
    s = s.replace("for b in blocks:", "for b in blocks[:SOURCE_LIMIT]:")

P.write_text(s, encoding='utf-8')
print('Priority patch OK | kaynak önceliği aktif | sıralı gönderim | canlı fiyat fallback | eşik %5')
''