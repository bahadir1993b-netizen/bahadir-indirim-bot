        ...
        try:
            ...
            for a in soup.find_all('a', href=True):
                ...
                for _ in range(5):
                    ...
                    if len(txt) > len(block):
                        block = txt
                    p = p.parent
                vals = _raw_card_prices((title + ' ' + block)[:5000])
                if len(vals) < 2:
                    continue
...
def _verify_with_initial_reference(page, site, url, fallback_title, expected_current):
    ...
    try:
        ...
        for selector in ['meta[property="product:price:amount"]','meta[itemprop="price"]','[itemprop="price"]','[data-price]']:
            for element in soup.select(selector):
                ...
                if x:
                    vals.append(x)
        if not vals:
            vals = prices(page_text)[:50]
        if not vals:
            vals = _raw_card_prices(page_text)[:50]
        if not vals:
            return None
        ...
        if discount < MIN_DISCOUNT or ref_previous / max(current, 1) > 4.0:
            return None
