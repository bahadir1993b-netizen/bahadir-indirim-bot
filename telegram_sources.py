...
COUPON_WORDS=re.compile(r'kupon|kod(?:u)?|sepette|kampanya|indirim|promosyon|ödeme anında|aktif|geçerli|gecerli|2 al 1|3 al 2|4 al 3',re.I)
...
def normalize(s,u):
    ...
    if s not in ('Amazon','Hepsiburada','Trendyol'):return None
...
def search_marketplace(page,s,title):
    ...
    try:
        page.goto(base+requests.utils.quote(q),wait_until='domcontentloaded',timeout=15000); page.wait_for_timeout(700)
        candidates=[]
        for a in page.locator('a[href]').all():
            u=clean(a.get_attribute('href') or '')
            if valid(s,u):candidates.append(u)
        if candidates:return normalize(s,candidates[0])
    except Exception:pass
    return None
...
def resolve(page,u,s,title):
    u=clean(u)
    try:
        r=requests.get(u,headers=HEAD,timeout=8,allow_redirects=True)
        f=clean(r.url); ss=site(f)
        if ss and valid(ss,f):return normalize(ss,f)
    except Exception:pass
    found=direct_from_page(page,u,s)
    if found:return found
    return search_marketplace(page,s,title)
...
def coupon_code(text):
    ...
    for pat in pats:
        for m in re.finditer(pat,text or '',re.I):
            ...
            if code.isdigit() or not re.search(r'[A-ZÇĞİÖŞÜ]',code):continue
            if code in {'INDIRIM','KAMPANYA','FIRSAT','AMAZON','HEPSIBURADA','TRENDYOL'}:continue
            return code
    return None
...
def collect_coupons():
    ...
    for source,channel in SOURCES.items():
        ...
        for b in soup.select('.tgme_widget_message'):
            ...
            st=next((x for x in ('Amazon','Hepsiburada','Trendyol') if re.search(r'\b'+re.escape(x)+r'\b',raw,re.I)),None)
            if st:out.append((st,code,raw,dt,source))
    return out

def match_coupon(s,title,raw,coupons):
    title_tokens={x for x in re.findall(r'[a-zçğıöşü0-9]{4,}',title.lower()) if x not in {'ürün','ürünü','hızlı','fırsat','indirim','adet','parça','set'}}
    best=None; score=0
    for cs,code,ctext,dt,source in coupons:
        if cs!=s:continue
        tokens={x for x in re.findall(r'[a-zçğıöşü0-9]{4,}',ctext.lower())}
        overlap=len(title_tokens & tokens)
        if code in raw.upper():overlap+=5
        if re.search(r'\b'+re.escape(code)+r'\b',raw,re.I):overlap+=5
...
def send(s,u,t,c,p,source,post_id,signal,coupon=None):
    if not valid(s,u):
        print(f'ATLANDI | geçersiz marketplace URL | {s} | {t[:70]}');return False
    ...
    if disc is None and not COUPON_WORDS.search(signal or ''):remember(key);return False
    ...
    print(f'GÖNDERİLDİ | {s} | {t[:70]} | {c:.2f} | kaynak={source} | link={u}'+(f' | kupon={coupon[0]}' if coupon else ''));return True
...
def parse(block,page,coupons):
    ...
    for a in block.select('a[href]'):
        original=clean(a.get('href','')); mapped=site(original)
    ...
    coupon=match_coupon(links[0][0],title,raw,coupons)
    return links[0][0],links[0][1],title[:300],c,p,post_id,signal,coupon
        ...
        try:
            for source,channel in SOURCES.items():
                ...
                for block in soup.select('.tgme_widget_message'):
                    ...
                    if item:candidates.append((source,item));print(f'Aday: {item[0]} | {item[2][:80]} | {item[3]:.2f} TL | kaynak={source}'+(f' | kupon={item[7][0]}' if item[7] else ''))
