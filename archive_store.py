import os,re,sqlite3,statistics,math
...
def key(title):
    ts=tokens(title)
    model=[x for x in ts if any(ch.isdigit() for ch in x) and any(ch.isalpha() for ch in x)]
    core=(model[:3]+[x for x in ts if x not in model][:7])[:10]
    return ' '.join(core)
...
def add(title,price,site='',old_price=None,source='',source_kind='archive',url='',recorded_at=None):
    ...
    dt=recorded_at or datetime.now(timezone.utc).isoformat()
    c=_conn();c.execute('INSERT OR IGNORE INTO title_prices(title_key,title,site,price,old_price,source,source_kind,product_url,recorded_at) VALUES(?,?,?,?,?,?,?,?,?)',
...
def _score_keys(query_key,row_key):
    ...
    inter=len(a&b);model_a={x for x in a if any(c.isdigit() for c in x)};model_b={x for x in b if any(c.isdigit() for c in x)}
    model_bonus=0.35 if model_a and model_a&model_b else 0
    return inter/max(1,len(a))+model_bonus

def history_by_title(title,days=365,limit=300):
    qk=key(title);cut=(datetime.now(timezone.utc)-timedelta(days=days)).isoformat()
    c=_conn();rows=c.execute('SELECT * FROM title_prices WHERE recorded_at>=? ORDER BY recorded_at DESC LIMIT 2500',(cut,)).fetchall();c.close()
    scored=[]
    for r in rows:
        s=_score_keys(qk,r['title_key'])
        if s>=0.55:scored.append((s,dict(r)))
    scored.sort(key=lambda x:(x[0],x[1]['recorded_at']),reverse=True)
...
def smart_reference(title,current,url_history=None,page_old=None,stored_old=None):
    """Conservative reference resistant to deal-post bias and long-term price inflation.
    Telegram observations are treated as deal-floor signals, never as the sole normal-price baseline.
    Older observations are recency weighted; product-specific trend adjusts stale history modestly.
    """
    now=datetime.now(timezone.utc);url_history=url_history or [];title_rows=history_by_title(title,365,400)
    ...
    for r in title_rows:
        ...
        age=max(0,(now-dt).days);kind=(r.get('source_kind') or '').lower();src=(r.get('source') or '').lower()
        all_rows.append((p,age,kind,src))
        op=r.get('old_price')
        if op:
            try:op=float(op)
            except:op=0
            if current*1.03<op<=current*2.2:explicit.append(op)
        if 'telegram' in kind or any(x in src for x in ['onual','firsat','enes','indirimdeal']):deals.append((p,age))
    ...
    for r in url_history:
        ...
        if 'telegram' in src:deals.append((p,age))
        else:strong.append((p,age))
    # Product-specific price drift: recent median vs older median, capped to avoid runaway inflation correction.
    ...
    if len(recent)>=2 and len(older)>=2:
        trend=max(0.85,min(1.35,_median(recent)/max(_median(older),1)))
    adjusted=[]
    for p,a in strong:
        if not (current*0.55<=p<=current*2.2):continue
        # Only a fraction of measured trend is applied, increasing with age.
        age_factor=min(1.0,a/180.0);adj=p*(1+(trend-1)*age_factor*0.65)
        adjusted.append(adj)
    # Explicit old/list prices are useful but clamp them to observed market/history when possible.
    if explicit:
        e=min(explicit)
        if adjusted:
            cap=statistics.quantiles(adjusted,n=4)[2] if len(adjusted)>=4 else max(adjusted)
            e=min(e,cap*1.12)
        if e>current*1.03:return e,'explicit-old'
    if len(adjusted)>=2:
        # lower-middle baseline: resistant to overpriced sellers but not distorted by deal-floor Telegram posts.
        vals=sorted(adjusted);ref=_median(vals)
        if len(vals)>=4:ref=min(ref,statistics.quantiles(vals,n=4)[2])
        if ref>current*1.03:return float(ref),'weighted-history'
    # Telegram/deal history can prove that today's price is NOT exceptional, but cannot alone create a high reference.
    if len(deals)>=2:
