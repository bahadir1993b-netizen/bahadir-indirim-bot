...
def sb(method, path, **kw):
    ...
    r = requests.request(method, f'{SB}/rest/v1/{path}', headers=headers, timeout=10, **kw); r.raise_for_status(); return r.json() if r.text else []
...
def normalize(site,url):
    p=urlparse(url); q=[(k,v) for k,v in parse_qsl(p.query,keep_blank_values=True) if k.lower() not in TRACKING]
    if site=='Amazon' and AMAZON_TAG:q.append(('tag',AMAZON_TAG))
    return urlunparse((p.scheme,p.netloc,p.path,p.params,urlencode(q,doseq=True),''))

def valid(site,url):
    try:
        p=urlparse(url); return p.netloc.lower().replace('www.','').endswith(MARKETS[site][1]) and bool(MARKETS[site][2].search(p.path))
    except: return False
...
def extract_search_candidates(page,site,query):
    page.goto(MARKETS[site][0]+quote(query),wait_until='domcontentloaded',timeout=8000); page.wait_for_timeout(350)
    ...
    for item in raw:
        ...
        if previous<=current or previous/max(current,1)>4:continue
        ...
        if len(title)>=10 and not BOOK_RE.search(title):out.append((url,title,current,previous))
        if len(out)>=4:break
...
def history(site,url,current):
    try:
        ...
        vals=[float(r['price']) for r in rows if r.get('price') is not None and float(r['price'])>current]; return min(vals) if vals else None
...
def verify(page,site,url,fallback_title,expected_current,candidate_previous):
    try:
        page.goto(url,wait_until='domcontentloaded',timeout=7000); page.wait_for_timeout(250); soup=BeautifulSoup(page.content(),'html.parser'); page_text=soup.get_text(' ',strip=True)
        ...
        if abs(current-expected_current)/max(expected_current,1)>.35:
            print(f'VERIFY FİYAT UYUŞMAZ | {site} | beklenen={expected_current:.2f} | sayfa={current:.2f}');return None
        old=history(site,url,current); previous=old if old else candidate_previous
        if not previous or previous<=current:record_price(site,url,current);return None
        if previous/current>4:record_price(site,url,current);return None
        discount=(previous-current)/previous*100; record_price(site,url,current)
        if discount<MIN_DISCOUNT:return None
        te=soup.select_one('meta[property="og:title"]') or soup.select_one('meta[name="twitter:title"]')
        title=te.get('content','').strip() if te else (soup.title.get_text(' ',strip=True) if soup.title else fallback_title)
...
def main():
    total=candidates_total=verified_total=0; seen_urls=set()
            ...
            for query in QUERIES:
                try:
                    ...
                    for url,title,current,previous in candidates:
                        ...
                        seen_urls.add(url); verified=verify(detail,site,url,title,current,previous)
                        if verified:
                            verified_total+=1;t,c,p,d,img=verified
