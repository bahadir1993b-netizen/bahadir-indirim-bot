...
QUERIES = ['indirim','fırsat','kampanya','elektronik','telefon','kulaklık','televizyon','ev yaşam','mutfak','kişisel bakım','bebek','oyuncak','spor','kozmetik']
...
def sb(method, path, **kw):
    ...
    r = requests.request(method, f'{SB}/rest/v1/{path}', headers=headers, timeout=15, **kw)
    r.raise_for_status()
...
def history(site, url, current):
    try:
        ...
        print('HISTORY HATA', site, e)
        return None


def record_price(site, url, current):
    try:
        sb('POST', 'price_history', json={'price': current, 'product_url': url, 'site': site, 'recorded_at': datetime.now(timezone.utc).isoformat()})
    except Exception as e:
        print('HISTORY KAYIT HATASI', site, e)


def verify(page, site, url, fallback_title, expected_current):
    try:
        ...
        if not current_vals:
            current_vals = prices(page_text)[:50]
        if not current_vals:
            return None
...
def send(site, url, title, current, previous, discount, image):
    ...
    if last:
        try:
            if datetime.now(timezone.utc) - datetime.fromisoformat(last.replace('Z', '+00:00')) < timedelta(hours=COOLDOWN):
                print(f'BAĞIMSIZ ATLANDI | cooldown | {title[:80]}')
                return False
        except Exception:
            pass
    fmt = lambda x: f'{x:,.2f} TL'.replace(',', 'X').replace('.', ',').replace('X', '.')
    text = f'🔥 %{discount:.0f} İNDİRİM\n\n🛍️ {title}\n💰 {fmt(current)}\n🏷️ Önceki: {fmt(previous)}\n\n👇 Fırsata git'
    keyboard = {'inline_keyboard': [[{'text':'🛒 FIRSATA GİT','url':url}]]}
    try:
        sent = False
        if image:
            r = requests.post(f'https://api.telegram.org/bot{TOKEN}/sendPhoto', json={'chat_id':CHAT,'photo':image,'caption':text,'reply_markup':keyboard}, timeout=15)
            sent = r.ok
        if not sent:
            requests.post(f'https://api.telegram.org/bot{TOKEN}/sendMessage', json={'chat_id':CHAT,'text':text,'disable_web_page_preview':False,'reply_markup':keyboard}, timeout=15).raise_for_status()
    except Exception as e:
        print('GÖNDERME HATASI', site, url, e)
        return False
    ...
    print(f'BAĞIMSIZ FIRSAT | {site} | %{discount:.1f} | {current:.2f} TL | {title[:90]}')
    return True
            ...
            for query in QUERIES:
                try:
                    ...
                    for url, title, current, previous in candidates:
                        verified = verify(detail, site, url, title, current)
                        if verified:
                            verified_total += 1
                            t, c, p, d, img = verified
                            if send(site, url, t, c, p, d, img):
                                total += 1
                except Exception as e:
