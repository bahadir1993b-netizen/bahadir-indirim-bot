...
MIN_DISCOUNT=6.0; COOLDOWN=12; AMAZON_TAG=os.getenv('AMAZON_ASSOCIATE_TAG','').strip(); HEAD={'User-Agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/131.0.0.0 Safari/537.36','Accept-Language':'tr-TR,tr;q=0.9,en;q=0.8'}
...
MARKETS={'Amazon':('https://www.amazon.com.tr/s?k=','amazon.com.tr',re.compile(r'/(?:dp|gp/product)/[A-Z0-9]{8,}',re.I)),'Hepsiburada':('https://www.hepsiburada.com/ara?q=','hepsiburada.com',re.compile(r'-p-[A-Za-z0-9]+(?:[/?#&]|$)',re.I)),'Trendyol':('https://www.trendyol.com/sr?q=','trendyol.com',re.compile(r'-p-\d+(?:[/?#&]|$)',re.I))}
...
def valid(site,u):
 try:
  p=urlparse(u); return p.netloc.lower().replace('www.','').endswith(MARKETS[site][1]) and bool(MARKETS[site][2].search(p.path))
 except:return False
def clean_title(text):
 text=' '.join((text or '').split()); text=re.sub(r'\b(?:Sepete ekle|Hızlı Teslimat|Çok Satan|Sponsorlu|Reklam)\b',' ',text,flags=re.I); return re.sub(r'\s{2,}',' ',text).strip()[:220]
def extract_card_data(site,href,anchor_text,card_text):
 if BOOK_RE.search(anchor_text+' '+card_text):return None
 all_prices=prices(card_text)
 if len(all_prices)<2:return None
 current=previous=None
 for pat in [r'(?:şimdi|şuan|şu an|güncel|satış|fiyatı?)\s*[:=]?\s*([\d.,]+)\s*(?:TL|₺)',r'([\d.,]+)\s*(?:TL|₺)\s*(?:yerine|şimdi)']:
  m=re.search(pat,card_text,re.I)
  if m:current=money(m.group(1));break
 for pat in [r'(?:önceki|eski|liste|normal|normalde)\s*[:=]?\s*([\d.,]+)\s*(?:TL|₺)',r'([\d.,]+)\s*(?:TL|₺)\s*(?:yerine|önce)']:
  m=re.search(pat,card_text,re.I)
 ...
 title=clean_title(anchor_text) if len(clean_title(anchor_text))>=10 else clean_title(card_text)
 if len(title)<10 or BOOK_RE.search(title):return None
 return disc,normalize(site,href),title,current,previous
def extract_search_candidates(page,site,query):
 page.goto(MARKETS[site][0]+quote(query),wait_until='domcontentloaded',timeout=12000); page.wait_for_timeout(1000)
 ...
 for item in raw:
  ...
  seen.add(u); data=extract_card_data(site,u,item.get('text') or '',item.get('card') or '')
  if data:out.append(data)
  if len(out)>=8:break
 print(f'BAĞIMSIZ ARAMA | {site} | "{query}" | aday={len(out)}'); return out
def explicit_old_prices(soup,page_text,current):
 old=[]
 for sel in ['del','s','.old-price','.list-price','.price-old','[class*="oldPrice"]','[class*="old-price"]','[class*="listPrice"]','[data-a-strike="true"]','[aria-label*="eski"]','[aria-label*="liste"]']:
  for e in soup.select(sel):
   x=money(e.get_text(' ',strip=True))
   if x and x>current:old.append(x)
 for pat in [r'(?:Liste Fiyatı|Liste fiyatı|Eski fiyat|Önceki fiyat|Normal fiyat|Tavsiye edilen satış fiyatı)\s*[:]?\s*([\d.,]+)\s*(?:TL|₺)',r'(?:was|list price|previous price|regular price)\s*[:]?\s*([\d.,]+)\s*(?:TL|₺)']:
...
def history(site,u,current):
 try:
  rows=sb('GET','price_history',params={'select':'price,recorded_at','product_url':f'eq.{u}','site':f'eq.{site}','order':'recorded_at.desc','limit':'30'})
  vals=[float(r['price']) for r in rows if r.get('price') is not None and float(r['price'])>current]
  return min(vals) if vals else None
 except Exception as e:print('HISTORY HATA',site,e);return None
def record_price(site,u,current):
...
def verify(page,site,u,fallback_title,expected_current,expected_previous):
 try:
  page.goto(u,wait_until='domcontentloaded',timeout=12000); page.wait_for_timeout(700); soup=BeautifulSoup(page.content(),'html.parser'); page_text=soup.get_text(' ',strip=True)
  if BOOK_RE.search((fallback_title or '')+' '+page_text[:5000]):return None
  title_el=soup.select_one('meta[property="og:title"]') or soup.select_one('meta[name="twitter:title"]'); title=title_el.get('content','').strip() if title_el else (soup.title.get_text(' ',strip=True) if soup.title else fallback_title)
  ...
  if not current_vals:current_vals=prices(page_text)[:50]
  ...
  if abs(current-expected_current)/max(expected_current,1)>0.35:
   print(f'VERIFY FİYAT UYUŞMAZ | {site} | beklenen={expected_current:.2f} | sayfa={current:.2f}');return None
  hist=history(site,u,current)
  old_explicit=explicit_old_prices(soup,page_text,current)
  previous=hist
  source='history' if hist else None
  if not previous and old_explicit:
   previous=min(old_explicit); source='explicit'
  # Arama kartındaki expected_previous artık kesinlikle eski fiyat kabul edilmiyor.
  if not previous or previous<=current:
   record_price(site,u,current)
   print(f'VERIFY GEÇMİŞ YOK | {site} | {current:.2f} TL | ilk gözlem, paylaşılmadı')
   return None
  ratio=previous/current
  if ratio>4.0:
   print(f'VERIFY ESKİ FİYAT ŞÜPHELİ | {site} | mevcut={current:.2f} | geçmiş={previous:.2f} | oran={ratio:.1f}x');record_price(site,u,current);return None
  disc=(previous-current)/previous*100
  if disc<MIN_DISCOUNT:
   record_price(site,u,current);return None
  record_price(site,u,current)
...
def send(site,u,title,current,previous,disc,image):
 rows=sb('GET','products',params={'select':'*','product_url':f'eq.{u}','limit':'1'}); row=rows[0] if rows else None; last=row.get('last_posted_at') if isinstance(row,dict) else None
 if last:
  try:
   if datetime.now(timezone.utc)-datetime.fromisoformat(last.replace('Z','+00:00'))<timedelta(hours=COOLDOWN):print(f'BAĞIMSIZ ATLANDI | cooldown | {title[:80]}');return False
  except:pass
 fmt=lambda x:f'{x:,.2f} TL'.replace(',','X').replace('.',',').replace('X','.'); text=f'🔥 %{disc:.0f} İNDİRİM\n\n🛍️ {title}\n💰 {fmt(current)}\n🏷️ Önceki: {fmt(previous)}\n\n👇 Fırsata git'; kb={'inline_keyboard':[[{'text':'🛒 FIRSATA GİT','url':u}]]}
 try:
  sent=False
  if image:
   r=requests.post('https://api.telegram.org/bot'+TOKEN+'/sendPhoto',json={'chat_id':CHAT,'photo':image,'caption':text,'reply_markup':kb},timeout=15);sent=r.ok
  if not sent:requests.post('https://api.telegram.org/bot'+TOKEN+'/sendMessage',json={'chat_id':CHAT,'text':text,'disable_web_page_preview':False,'reply_markup':kb},timeout=15).raise_for_status()
 except Exception as e:print('GÖNDERME HATASI',site,u,e);return False
 ...
 else:sb('POST','products',json={'product_name':title,'current_price':current,'previous_price':previous,'product_url':u,'site':site,'updated_at':now,'last_posted_at':now})
   ...
   for query in QUERIES:
    try:
     ...
     for disc,u,title,current,old in candidates:
      ...
      if v:
       verified_total+=1;t,c,p,d,img=v
