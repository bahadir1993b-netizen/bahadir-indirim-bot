...
def parse_direct(line,items,run):
    ...
    m=re.match(r'^KONTROL:\s*([^|]+)\|\s*([\d.]+)\s*->\s*ref\s*([\d.]+)\s*\|\s*%([\d.]+).*?\|\s*ref=([^|]+)\s*\|\s*geçmiş=(\d+)\s*\|\s*(.+)$',s)
    if m:
        site,cur,ref,disc,rsrc,hist,title=m.groups();row=r.ensure(items,title.strip(),site.strip());row.update({'current_price':float(cur),'reference_price':float(ref),'discount_pct':float(disc),'history_count':int(hist),'reference_source':rsrc.strip()});row['events'].append('direct_check');return
    m=re.match(r'^REFERANS YOK:\s*([^|]+)\|\s*([\d.]+)\s*\|\s*lokal_geçmiş=(\d+)\s*\|\s*(.+)$',s)
    if m:
    ...
    if s.startswith('=== API-SİZ V3 BİTTİ'):
...
def main():
    ...
    run={'started_at':datetime.now(timezone.utc).isoformat(),'report_version':6,'engine':'run_direct_watch_v3.py','mode':'api-free-v3-local-memory'};items={};start=time.time()
    ...
    txt_path.write_text(r.human_report(run,items,raw_path).replace('SERPER V2 TUR RAPORU','API-SİZ FİYAT TAKİP V3 + VPS FİYAT HAFIZASI'),encoding='utf-8')
    ...
    if r.REPORT_CHAT_ID:
        r.send_report(txt_path,'API-siz V3 + VPS fiyat hafızası raporu');r.send_report(json_path,'API-siz V3 JSON raporu')
