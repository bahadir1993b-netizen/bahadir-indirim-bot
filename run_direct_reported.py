...
def main():
    ...
    run={'started_at':datetime.now(timezone.utc).isoformat(),'report_version':4,'engine':'run_direct_watch.py','mode':'api-free'};items={};start=time.time()
    with raw_path.open('w',encoding='utf-8') as raw:
        p=subprocess.Popen(['python','-u','run_direct_watch.py'],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,bufsize=1)
    ...
    txt_path.write_text(r.human_report(run,items,raw_path).replace('SERPER V2 TUR RAPORU','API-SİZ FİYAT TAKİP RAPORU'),encoding='utf-8')
    ...
    if r.REPORT_CHAT_ID:
        r.send_report(txt_path,'API-siz fiyat takip tur raporu');r.send_report(json_path,'API-siz fiyat takip JSON raporu')
