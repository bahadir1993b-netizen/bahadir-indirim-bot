...
SOURCE_SITE={'Amazon Özel':'Amazon','amazonozel':'Amazon','OnuAl':None,'onual_firsat':None}
...
def sb(method,path,**kwargs):
 ...
 r=requests.request(method,f'{SB}/rest/v1/{path}',headers=h,timeout=15,**kwargs);r.raise_for_status();return r.json() if r.text else []
...
def site_of_link(block):
 ...
 return None
def infer_site(source,channel,raw,block):
 return site_of_text(raw) or site_of_link(block) or SOURCE_SITE.get(source) or SOURCE_SITE.get(channel)
def valid(site,u):
...
def conditions(raw,code):
 parts=re.split(r'\n+|(?<=[.!?])\s+',raw);keys=('alt limit','minimum','min.','en az','üzeri','uzeri','üzerine','uzerine','sepet','adet','ürün','urun','kategori','marka','geçerli','gecerli','kod','kupon','kampanya','alışveriş','alisveris','hariç','haric','aktif','mobil','uygulama')
 out=[]
 for part in parts:
 ...
 return out[:5]
def active_signal(raw):return bool(re.search(r'aktif|mevcut|geçerli|gecerli|kullanabilirsiniz|kullanılır|kullanilir|başladı|basladi|sepette|uygulamada|mobil|kampanya|kupon|kod',raw,re.I))
def fetch(source,channel):
 r=requests.get(f'https://t.me/s/{channel}',headers=HEADERS,timeout=20);print(f'Kupon kaynağı {source}: HTTP {r.status_code}')
 ...
 for block in soup.select('.tgme_widget_message'):
  ...
  site=infer_site(source,channel,raw,block)
  if site not in SITES:
   print(f'KUPON ATLANDI | {source} | {code} | marketplace bulunamadı');continue
  if not active_signal(raw):continue
