...
def normalize(site,u):
 ...
 u=clean(u);p=urlparse(u);pairs=[(k,v) for k,v in parse_qsl(p.query,keep_blank_values=True) if k.lower() not in TRACKING]
...
def product_link(block,site):
 for a in block.find_all('a',href=True):
  u=clean(a['href'])
  if site=='Amazon' and ('amazon.com.tr' in u or 'amzn.' in u or 'link.amazon' in u):return normalize(site,u)
  if site=='Hepsiburada' and ('hepsiburada.com' in u or 'hb.biz' in u or 'hps.im' in u):return normalize(site,u)
  if site=='Trendyol' and ('trendyol.com' in u or 'ty.gl' in u or 'tyml.gl' in u):return normalize(site,u)
 return None
...
def fetch(source,channel):
 ...
 for block in soup.select('.tgme_widget_message'):
  ...
  link=product_link(block,site);prices=money_values(raw);disc=discount_amount(raw);minimum=min_spend(raw);cond=conditions(raw,code)
  active=bool(re.search(r'aktif|mevcut|geçerli|gecerli|sepette|uygulamada|mobil|kullanabilirsiniz|kullanılır|kullanilir|başladı|basladi',raw,re.I))
  if not active and not re.search(r'\b(?:KOD|KODU|KUPON)\b',raw,re.I):continue
  ...
  if link:lines += ['', '👇 Fırsata git']
  else:lines += ['', '👇 Kampanyaya git']
  ...
  print(f'KUPON GÖNDERİLDİ | {site} | {code} | kaynak={source}')
 return sent
def main():
 total=0
