...
def extract_code(text):
    patterns=[
        r'\b(?:KOD|KODU|KUPON|KUPON KODU|PROMOSYON(?: KODU)?)\s*[:=\-]?\s*([A-ZÇĞİÖŞÜ0-9][A-ZÇĞİÖŞÜ0-9_-]{3,23})\b',
...
def site_of_link(block):
    for a in block.find_all('a',href=True):
        ...
        if host in ('publicis.link','onu.al'):
            try:
                r=requests.get(u,headers=HEADERS,timeout=8,allow_redirects=True)
                final=r.url.lower()
                if 'amazon.com.tr' in final:return 'Amazon'
                if 'hepsiburada.com' in final:return 'Hepsiburada'
                if 'trendyol.com' in final:return 'Trendyol'
            except: pass
    return None

def money_values(text):
    return [m.group(0).strip() for m in re.finditer(r'(?<![A-ZÇĞİÖŞÜ])(?:\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{1,2})?|\d+(?:[.,]\d{1,2})?)\s*(?:tl|₺)(?![A-ZÇĞİÖŞÜ])',text,re.I)]
...
def product_link(block,site):
    for a in block.find_all('a',href=True):
        u=a['href']
        if site=='Amazon' and ('amazon.com.tr' in u or 'amzn.' in u or 'link.amazon' in u or 'publicis.link' in u):return u
