...
# Tek eşik: varsayılan %15. .env MIN_DISCOUNT ile değiştirilebilir.
...
MAX_RESOLVE_PER_RUN=int(os.environ.get('MAX_RESOLVE_PER_RUN','12'))
CACHE_FILE=Path('/app/data/link_cache.json');CACHE_FILE.parent.mkdir(parents=True,exist_ok=True)
_resolve_count=0
STATS={'seen':0,'no_price':0,'no_link':0,'no_ref':0,'below':0,'cooldown':0,'sent':0,'errors':0,'amazon':0,'hepsiburada':0,'trendyol':0}
...
def valid_product_url(link,site):
    try:
        ...
        if TARGETS.get(host)!=site or not path or low.endswith('/yorumlari') or '/kategori/' in low:return False
...
def result_matches_title(item_title,result_title,link):
    ...
    return bool(a) and (len(common)>=2 or (len(a)<=2 and len(common)>=1))
...
def resolve_direct_link(item,site):
    ...
    print(f'Güvenilir direkt ürün linki bulunamadı; 12 saat cachelendi: {site} | {title[:90]}');cache_set(site,title,None);return None
...
def process_item(item):
    ...
    db_url=product_identity(item,site,direct_link);image=product_photo(direct_link,item.get('imageUrl') or '');now=datetime.now(timezone.utc).isoformat()
    try:
        rows=bot.sb('GET','products',params={'select':'*','product_url':f'eq.{db_url}','limit':'1'});old=bot.history(db_url)
        hist_ref=max((x for x in old if x>current*1.03),default=None);source_ref=item_reference_price(item,current);prev=max([x for x in (hist_ref,source_ref) if x],default=None)
...
def main():
    global _resolve_count;_resolve_count=0
    print(f'=== Serper alışveriş botu başladı | eşik=%{bot.MIN_DISCOUNT:g} ===')
    seen=set();matched=0
    for q in QUERIES:
    ...
    print(f'=== Bitti. Hedef={matched} | Amazon={STATS["amazon"]} HB={STATS["hepsiburada"]} Trendyol={STATS["trendyol"]} | fiyat_yok={STATS["no_price"]} link_yok={STATS["no_link"]} referans_yok={STATS["no_ref"]} esik_alti={STATS["below"]} cooldown={STATS["cooldown"]} hata={STATS["errors"]} | link_sorgu={_resolve_count} | Gönderilen={STATS["sent"]} ===')
