...
def page_info(link,serper_current,serper_title):
    ...
    try:
        ...
        for sel,attr in [('meta[property="og:title"]','content'),('h1',None),('title',None)]:
            e=soup.select_one(sel)
            if e:
                t=(e.get(attr) if attr else e.get_text(' ',strip=True)) or ''
                t=re.sub(r'\s+',' ',t).strip()
                if len(t)>=8 and matches(serper_title,t,link):out['title']=t[:300];break
        live=[];old=[]
        for sel,attrs in [
            ('meta[itemprop="price"]',['content']),('meta[property="product:price:amount"]',['content']),('[itemprop="price"]',['content','data-price']),
            ('.a-price .a-offscreen',[None]),('[data-test-id="price-current-price"]',[None]),('[class*="currentPrice"]',[None]),('[class*="salePrice"]',[None])]:
            for e in soup.select(sel):
                for a in attrs:
                    p=parse_price(e.get(a) if a else e.get_text(' ',strip=True))
                    if p and serper_current*0.5<=p<=serper_current*1.5:live.append(p)
    ...
    try:
        ...
        if market_med:
            if market_floor and current>market_floor*1.05:
                ...
                STATS['market_blocked']+=1
                print(f'PİYASA ENGELİ: {site} | medyana göre indirim yetersiz | {title[:65]}')
                prev=None
