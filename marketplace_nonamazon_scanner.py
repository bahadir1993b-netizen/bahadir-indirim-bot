...
SITES={
 'Hepsiburada':('https://www.hepsiburada.com/ara?q=',re.compile(r'-p-[A-Za-z0-9]+(?:[/?#&]|$)',re.I)),
 'Trendyol':('https://www.trendyol.com/sr?q=',re.compile(r'-p-\d+(?:[/?#&]|$)',re.I)),
}
...
def extract(page,site,query):
 base,pat=SITES[site]
 page.goto(base+quote(query),wait_until='domcontentloaded',timeout=10000);page.wait_for_timeout(700)
 ...
 for x in items:
  ...
  seen.add(u)
  vals=[money(m.group()) for m in MONEY.finditer(card)]
  ...
  out.append((u,title,current,previous))
  if len(out)>=5:break
...
def verify(page,site,u,expected,previous):
 try:
  page.goto(u,wait_until='domcontentloaded',timeout=8000);page.wait_for_timeout(500)
  ...
  for el in soup.select('script[type="application/ld+json"]'):
   ...
   for m in re.finditer(r'"price"\s*:\s*"?([0-9]+(?:[.,][0-9]+)?)',txt):
    v=money(m.group(1));
    if v:vals.append(v)
 ...
 try:
  if image:
   ...
   if r.ok and len(r.content)>1000:
    requests.post(f'https://api.telegram.org/bot{TOKEN}/sendPhoto',data={'chat_id':CHAT,'caption':text,'reply_markup':__import__('json').dumps(kb,ensure_ascii=False)},files={'photo':('product.jpg',r.content,'image/jpeg')},timeout=15).raise_for_status();return True
...
def main():
 ...
 with sync_playwright() as pw:
  ...
  page=browser.new_page(user_agent=HEAD['User-Agent'],locale='tr-TR');detail=browser.new_page(user_agent=HEAD['User-Agent'],locale='tr-TR')
  for site in SITES:
   for q in QUERIES:
    try:
     for u,title,current,old in extract(page,site,q):
      ...
      if not v:continue
      t,c,p,d,img=v
      if send(site,u,t,c,p,d,img):
       sent+=1;print(f'⭐️ NON-AMAZON FIRSAT | {site} | %{d:.1f} | {c:.2f} TL | {t[:80]}')
