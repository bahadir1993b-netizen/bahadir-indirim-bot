...
def key(title):
    return re.sub(r'\s+',' ',(title or '').strip().lower())[:180]


def ensure(items,title,site=''):
    k=key(title)
    if not k:k=f'unknown-{len(items)+1}'
    row=items.setdefault(k,{'title':title.strip() if title else '', 'site':site, 'events':[]})
...
def parse_line(line,items,run):
    ...
    if m:
        site,current,ref,hist,src,title=m.groups();r=ensure(items,title,site.strip())
        r.update({'current_price':float(current),'reference_price':float(ref),'history_count':int(hist),'reference_source':src.strip()})
        r['events'].append('kontrol');return
    m=re.search(r'^PİYASA KONTROL:\s*([^|]+)\|\s*ürün=([\d.]+)\s*\|\s*en_ucuz=([\d.]+)\s*\|\s*medyan=([\d.]+)\s*\|\s*n=(\d+)\s*\|\s*(.+)$',s)
    if m:
        site,cur,floor,med,n,title=m.groups();r=ensure(items,title,site.strip())
        r.update({'market_floor':float(floor),'market_median':float(med),'market_samples':int(n)})
        r['events'].append('piyasa_kontrol');return
    m=re.search(r'^PİYASA ENGELİ:\s*([^|]+)\|\s*(.*?)\|\s*(.+)$',s)
    if m:
        site,reason,title=m.groups();r=ensure(items,title,site.strip());r['result']='GÖNDERİLMEDİ';r['reason']='PİYASA_ENGELİ: '+reason.strip();r['events'].append('piyasa_engeli');return
    ...
    m=re.search(r'^BAŞLIK ZENGİNLEŞTİ:\s*(.+)$',s)
    if m:ensure(items,m.group(1))['events'].append('baslik_zenginlesti');return
    ...
    m=re.search(r'^Link cache kullanıldı:\s*([^|]+)\|\s*(https?://\S+)',s)
    if m:run.setdefault('cached_links',[]).append({'site':m.group(1).strip(),'url':m.group(2)});return
    ...
    if s.startswith('=== Bitti.'):
        run['summary_line']=s
        pairs=re.findall(r'([A-Za-z_çğıöşüÇĞİÖŞÜ]+)=([\d.]+)',s)
        run['summary']={k:(float(v) if '.' in v else int(v)) for k,v in pairs}


def human_report(run,items,raw_path):
    sm=run.get('summary',{})
    lines=[
        'BAHADIR İNDİRİM BOTU - SERPER TUR RAPORU',
        '='*55,
        f"Başlangıç: {run.get('started_at','')}",
        f"Bitiş: {run.get('finished_at','')}",
        f"Süre: {run.get('duration_seconds',0):.1f} sn",
        '',
        'ÖZET',
        '-'*55,
        run.get('summary_line','Özet satırı bulunamadı.'),
        '',
        'ÜRÜNLER / TEŞHİS',
        '-'*55,
    ]
    rows=list(items.values())
    rows.sort(key=lambda x:(x.get('result')!='GÖNDERİLDİ',x.get('site',''),x.get('title','')))
    for i,r in enumerate(rows,1):
        lines += [f'[{i}] {r.get("site","")} | {r.get("title","")}',
                  f'  Fiyat: {r.get("current_price",r.get("page_price",r.get("serper_price","?")))}',
                  f'  Referans: {r.get("reference_price","?")} | kaynak={r.get("reference_source","?")}',
                  f'  Piyasa: en_ucuz={r.get("market_floor","?")} medyan={r.get("market_median","?")} n={r.get("market_samples","?")}',
                  f'  Stok HTTP: {r.get("stock_http","?")} | Render: {r.get("stock_render","?")}',
                  f'  Sonuç: {r.get("result","İNCELENDİ")} | Neden: {r.get("reason","-")}',
                  f'  URL: {r.get("url","-")}',
                  f'  Olaylar: {", ".join(r.get("events",[])) or "-"}', '']
    lines += ['HAM LOG', '-'*55, f'Bakınız: {raw_path.name}']
    return '\n'.join(lines)
...
def send_report(path,caption):
    ...
    try:
        with path.open('rb') as f:
            r=requests.post(f'https://api.telegram.org/bot{BOT_TOKEN}/sendDocument',data={'chat_id':REPORT_CHAT_ID,'caption':caption[:900]},files={'document':(path.name,f)},timeout=30)
...
def main():
    ...
    raw_path=REPORT_DIR/f'{stamp}_raw.log';json_path=REPORT_DIR/f'{stamp}.json';txt_path=REPORT_DIR/f'{stamp}.txt'
    run={'started_at':datetime.now(timezone.utc).isoformat(),'report_version':1}
    items={};start=time.time()
    with raw_path.open('w',encoding='utf-8') as raw:
        p=subprocess.Popen(['python','-u','run_serper.py'],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,bufsize=1)
        for line in p.stdout:
            print(line,end='');raw.write(line);raw.flush();parse_line(line,items,run)
        code=p.wait()
    run['exit_code']=code;run['finished_at']=datetime.now(timezone.utc).isoformat();run['duration_seconds']=round(time.time()-start,2)
    # Infer simple outcomes for rows that reached a valid reference but lack explicit reason.
    for r in items.values():
        cur=r.get('current_price');ref=r.get('reference_price')
        if cur and ref and ref>cur and 'result' not in r:r['discount_pct']=round((ref-cur)/ref*100,2)
    payload={'run':run,'items':list(items.values())}
    json_path.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding='utf-8')
    txt_path.write_text(human_report(run,items,raw_path),encoding='utf-8')
    ...
    if REPORT_CHAT_ID:
        send_report(txt_path,'Serper tur teşhis raporu')
        send_report(json_path,'Serper tur JSON raporu')
