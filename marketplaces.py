...
def unwrap(u):
    ...
    for _ in range(8):
        ...
        for k in ('q','url','uddg','target','dest','destination'):
            if q.get(k):
                candidate=unquote(q[k][0])
                if candidate.startswith(('http://','https://')):nxt=candidate;break
...
def add_candidate(site,href,title,text,out,seen):
    ...
    seen.add(u)
    ps=prices(text)
    out.append((u,(title or 'Ürün').strip()[:250],ps))
...
def search_engine(site,term,engine):
    ...
    url={'bing':f'https://www.bing.com/search?q={q}&count=30','yahoo':f'https://search.yahoo.com/search?p={q}','google':f'https://www.google.com/search?q={q}&num=20&filter=0'}[engine]
    out=[];seen=set()
    try:
        ...
        blocks=soup.select('li.b_algo, div.MjjYud, div.g, .result, .web-result, .algo')
        for block in blocks:
        ...
        # Arama motoru HTML'sindeki çıplak/relative ürün linklerini de tara.
        for href in re.findall(r'(?:https?:)?//(?:www\.)?'+re.escape(domain)+r'/[^"\'<>\s]+-p-[A-Za-z0-9]+|/[^"\'<>\s]+-p-[A-Za-z0-9]+',raw,re.I):
...
def direct_search(site,term,browser):
    url=SEARCH_URLS[site].format(q=quote(term)); out=[];seen=set(); page=browser.new_page(); page.set_default_timeout(7000); page.set_default_navigation_timeout(20000)
    try:
        ...
        page.wait_for_timeout(1800); html=page.content(); soup=BeautifulSoup(html,'html.parser')
        for a in soup.find_all('a',href=True):
            txt=re.sub(r'\s+',' ',a.get_text(' ',strip=True)); add_candidate(site,a.get('href'),txt,' '.join([txt,a.get('aria-label','') or '']),out,seen)
            if len(out)>=MAX_PRODUCTS:break
        if len(out)<MAX_PRODUCTS:
            raw=htmlmod.unescape(html).replace('\\/','/').replace('\\u002F','/')
            for href in re.findall(r'(?:https?:)?//(?:www\.)?'+re.escape(SITES[site])+r'/[^"\'<>\s]+-p-[A-Za-z0-9]+|/[^"\'<>\s]+-p-[A-Za-z0-9]+',raw,re.I):
...
def discover(site,browser):
    found=[];seen=set()
    # Siteler GitHub Actions IP'sini 403 ile engelliyorsa doğrudan aramayı zorlamıyoruz;
    # arama motoru sonuçları birincil keşif kaynağı oluyor.
    for term in TERMS:
        ...
        if len(found)>=MAX_PRODUCTS:return found
    # Arama motorları tamamen boş kalırsa son çare kendi arama sayfası.
    if not found:
        for term in TERMS:
...
def jsonld_prices(html):
    ...
    return name,list(dict.fromkeys(vals))
...
def product_page(site,url,title,browser):
    ...
    try:
        ...
        page.wait_for_timeout(1500); html=page.content(); name,jd=jsonld_prices(html)
        if site=='Trendyol':
            current=first_price(page,['meta[property="product:price:amount"]','meta[itemprop="price"]','[data-testid="price-current"]','[data-testid="price"]','[class*="prc-dsc"]','[class*="price-current"]'])
            old=first_price(page,['[class*="prc-org"]','[class*="price-original"]','[class*="original-price"]','[class*="strike"]'])
        else:
            current=first_price(page,['meta[property="product:price:amount"]','meta[itemprop="price"]','[data-test-id="price-current"]','[data-test-id="current-price"]','[class*="product-price"]','[class*="current-price"]'])
        ...
        if current is None:
            text=page.locator('body').inner_text(timeout=5000); vals=prices(text); current=vals[0] if vals else None
