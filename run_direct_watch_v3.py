...
def merge_catalog():
    ...
    try:
        for r in v2.load_catalog():
            ...
            else:r=dict(r);r['product_url']=u;merged[u]=r
...
def send(row,current,ref,title,image,site,refsrc,campaign=None):
    ...
    if campaign:
        ...
        if campaign.get('qty'):lines.append(f'📦 {campaign["qty"]} adet toplam yaklaşık: {fmt(current*float(campaign["qty"]))} TL')
    lines += [f'🏷️ Normal fiyat: {fmt(ref)} TL',f'🛍️ {site}','','👇 Fırsata git'];kb={'inline_keyboard':[[{'text':'🛒 FIRSATA GİT','url':url}]]};text='\n'.join(lines);resp=None
    if image:
        try:
...
def main():
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            browser.close()
        ls.runtime_finish('direct','ok' if STATS['errors']==0 else 'warning',candidates=STATS['catalog'],checked=STATS['checked'],sent=STATS['sent'],errors=STATS['errors'],details=STATS)
        print('=== API-SİZ V5 BİTTİ | '+' | '.join(f'{k}={v}' for k,v in STATS.items())+' ===')
    except Exception as e:
        STATS['errors']+=1;ls.runtime_finish('direct','error',candidates=STATS['catalog'],checked=STATS['checked'],sent=STATS['sent'],errors=STATS['errors'],details={'error':type(e).__name__});raise
