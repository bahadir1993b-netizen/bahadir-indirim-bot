...
raw='https://www.amazon.com.tr/dp/B0TEST1234?tag=wrong&ref=x'
u=pc.affiliate_url(raw)
check('tag=ozelfirsat09-21' in u,'affiliate tag missing')
check('tag=wrong' not in u,'old affiliate tag survived')
check(pc.affiliate_ok(u),'affiliate verification failed')
check(not pc.affiliate_ok(raw),'wrong affiliate tag accepted')
...
check(ls.publication_key('https://www.trendyol.com/a-p-123456')==ls.publication_key('https://www.trendyol.com/b-p-123456?merchantId=1'),'trendyol duplicate identity')
